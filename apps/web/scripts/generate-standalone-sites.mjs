...
function renderLessonButton(seriesId, lessonNumber, title = `第 ${lessonNumber} 课`) {
  const lessonId = `${seriesId}-${String(lessonNumber).padStart(2, '0')}`;
  const videoUrl = `${SERIES[seriesId].base ?? releaseBase}/${lessonId}.mp4`;
...
function renderPage(seriesId) {
  ...
  const lessons = Array.from({ length: series.count }, (_, index) =>
    renderLessonButton(seriesId, index + 1, series.lessonTitles?.[index])
  ).join('\n');
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:"Microsoft YaHei","PingFang SC",system-ui,sans-serif;line-height:1.55}
    ...
    .player{overflow:hidden;border:1px solid var(--border);border-radius:16px;background:#000;box-shadow:var(--shadow)}.player-stage{position:relative;background:#000}video{display:block;width:100%;max-height:72vh;aspect-ratio:16/9;background:#000}.player-cover{position:absolute;inset:0;z-index:1;display:grid;place-items:center;border:0;padding:0;background:#000;cursor:pointer}.player-cover.hidden{display:none}.player-cover img{width:100%;height:100%;object-fit:cover}.player-cover span{position:absolute;left:50%;top:50%;display:grid;place-items:center;width:72px;height:72px;transform:translate(-50%,-50%);border-radius:50%;background:rgba(0,0,0,.55);color:#fff;font-size:2rem;text-shadow:0 2px 12px rgba(0,0,0,.75)}.player-footer{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:14px 18px;background:var(--surface)}.player-hint{color:var(--muted)}
...
${series.archive ? `      <a class="button" href="${series.archive}" download>下载全部课程</a>` : ''}
    ...
    <section class="player" aria-label="课程播放器">
      ...
      <div class="player-footer">
        ...
        <a id="lesson-download" href="${series.base ?? releaseBase}/${firstLessonId}.mp4" download>下载本节课视频</a>
      </div>
    </section>
  ...
  <script>
    (() => {
      ...
      function selectLesson(button) {
        ...
        playerCoverImage.src = button.dataset.cover;
